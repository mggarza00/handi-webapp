-- Tabla de solicitudes de cambio de perfil
...
create index if not exists profile_change_requests_user_status_idx on public.profile_change_requests (user_id, status);

alter table public.profile_change_requests enable row level security;

-- Políticas de acceso
-- El autor puede ver sus propias solicitudes; los admins pueden ver todas
...
-- Crear solicitud: el usuario autenticado solo para sí mismo
...
-- Los usuarios no pueden actualizar sus solicitudes; solo admins
...
-- Los admins pueden borrar (opcional)
