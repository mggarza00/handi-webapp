...
export async function notifyApplicationCreated(params: { request_id: string; professional_id: string }) {
  const admin = getAdmin();
  if (!admin) return;
  ...
  const clientId = (req as unknown as { created_by?: string } | null | undefined)?.created_by;
  if (!clientId) return;
  ...
  const reqTitle = (req as unknown as { title?: string } | null | undefined)?.title;
  const subject = "Nueva postulación a tu solicitud";
  const html = applicationCreatedHtml({ requestTitle: reqTitle, linkUrl });
  await sendEmail({ to: email, subject, html });
}

export async function notifyApplicationUpdated(params: { application_id: string; status: string }) {
  const admin = getAdmin();
  if (!admin) return;
  ...
  const proId = (app as unknown as { professional_id?: string } | null | undefined)?.professional_id;
  const reqId = (app as unknown as { request_id?: string } | null | undefined)?.request_id;
...
export async function notifyAgreementCreated(params: { request_id: string; professional_id: string; agreement_id: string }) {
  const admin = getAdmin();
  if (!admin) return;
  ...
  const base = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
  const linkUrl = `${base}/requests/${request_id}`;
  const reqTitle = (req as unknown as { title?: string } | null | undefined)?.title ?? request_id;
  const subject = `Nuevo acuerdo #${agreement_id.slice(0, 8)}`;
  const html = agreementCreatedHtml({ requestTitle: reqTitle, agreementIdShort: agreement_id.slice(0, 8), linkUrl });
  if (clientEmail) await sendEmail({ to: clientEmail, subject, html });
  if (proEmail) await sendEmail({ to: proEmail, subject, html });
}

export async function notifyAgreementUpdated(params: { agreement_id: string; status: string }) {
  const admin = getAdmin();
  if (!admin) return;
  ...
  const reqId = (agr as unknown as { request_id?: string } | null | undefined)?.request_id;
  const proId = (agr as unknown as { professional_id?: string } | null | undefined)?.professional_id;
  if (!reqId) return;
  ...
  const base = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
  const linkUrl = `${base}/requests/${reqId}`;
  const reqTitle = (req as unknown as { title?: string } | null | undefined)?.title ?? reqId;
  const subject = `Acuerdo actualizado: ${status}`;
  const html = agreementUpdatedHtml({ requestTitle: reqTitle, agreementIdShort: agreement_id.slice(0, 8), status, linkUrl });
  if (clientEmail) await sendEmail({ to: clientEmail, subject, html });
  if (proEmail) await sendEmail({ to: proEmail, subject, html });
}

export async function notifyMessageReceived(params: { request_id: string; to_user_id: string; text: string }) {
  const admin = getAdmin();
  if (!admin) return;
  ...
  const html = messageReceivedHtml({ requestTitle: reqTitle, preview, linkUrl });
  await sendEmail({ to: email, subject, html });
}
