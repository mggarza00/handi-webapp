...
export async function getInterFont(): Promise<ArrayBuffer> {
  ...
  for (const p of candidates) {
    try {
      ...
      interData = buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength);
      try { globalThis.__SATORI_FONT_PATH__ = p; } catch { /* ignore */ }
      if (process.env.SATORI_FONT_DEBUG === '1' || process.env.NODE_ENV !== 'production') {
  ...
  throw new Error("No se encontró una fuente TTF compatible (Inter). Agrega /public/fonts/Inter-VariableFont.ttf.");
}

// Site font exports used by app/layout.tsx
export const nunito = Nunito({ subsets: ["latin"], variable: "--font-nunito" });
export const varelaRound = Varela_Round({ subsets: ["latin"], weight: "400", variable: "--font-varela" });
export const concertOne = Concert_One({ subsets: ["latin"], weight: "400", variable: "--font-concert" });
