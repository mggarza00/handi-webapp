...
export async function getProProfile(userId: string): Promise<{ full_name: string | null; avg_rating: number }> {
  try {
    ...
    const avg_rating = typeof prof?.rating === "number" ? (prof.rating as number) : 0;
    return { full_name, avg_rating };
  } catch {
    return { full_name: null, avg_rating: 0 };
  }
}

export async function getJobsInProgress(userId: string, limit = 5): Promise<Array<{ id: string; request_id: string; title: string; status: string | null; updated_at: string | null }>> {
  try {
    const supa = createClient();
    ...
    if (ids.length > 0) {
      const { data: reqs } = await (supa as any)
        ...
        .select("id, title")
        .in("id", ids);
      titlesByReq = new Map(((reqs as any[]) || []).map((r) => [String(r.id), String(r.title || "")]));
    }
...
export async function getJobsCompleted(userId: string, limit = 5): Promise<Array<{ id: string; request_id: string; title: string; completed_at: string | null }>> {
  try {
    const supa = createClient();
    const { data, error } = await (supa as any)
      .from("agreements")
...
export async function getPotentialJobs(userId: string, limit = 5): Promise<Array<{ id: string; title: string; city: string | null; category: string | null; created_at: string | null }>> {
  try {
    const { items } = await fetchExploreRequests(userId, { page: 1, pageSize: Math.max(1, limit) });
    return (items || []).map((it) => ({ id: String(it.id), title: it.title, city: it.city ?? null, category: it.category ?? null, created_at: it.created_at ?? null }));
  } catch {
...
function formatLabel(date: Date, interval: Interval, _indexInRange: number): string {
  ...
  if (interval === "week") {
    ...
    const diffDays = Math.floor((monday.getTime() - startOfWeek(jan1).getTime()) / (1000 * 60 * 60 * 24));
...
function getPeriodBoundaries(interval: Interval, periods = 6): Array<{ from: Date; to: Date; label: string }> {
  const out: Array<{ from: Date; to: Date; label: string }> = [];
  const now = new Date();
  let cursor: Date;
  if (interval === "week") cursor = startOfWeek(now);
  else if (interval === "fortnight") {
    cursor = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() <= 15 ? 1 : 16));
  ...
  for (let i = 0; i < periods; i++) {
    ...
    if (interval === "week") {
      ...
      if (from.getUTCDate() === 1) cursor = new Date(Date.UTC(from.getUTCFullYear(), from.getUTCMonth() - 1, 16));
      else cursor = new Date(Date.UTC(from.getUTCFullYear(), from.getUTCMonth(), 1));
    } else {
      cursor = new Date(Date.UTC(from.getUTCFullYear(), from.getUTCMonth() - 1, 1));
    }
  }
  return out;
}

export async function getEarningsSeries(userId: string, interval: Interval): Promise<EarningsPoint[]> {
  try {
    const supa = createClient();
    ...
    let payments: Array<{ amount: number; created_at: string; paid_at?: string | null; request_id?: string | null }> = [];
    try {
    ...
    if (!Array.isArray(payments) || payments.length === 0) {
      ...
      const reqIds = Array.from(new Set(((agrs as any[]) || []).map((a) => String(a.request_id))));
      if (reqIds.length > 0) {
    ...
    return ranges.map((r) => ({ label: r.label, amount: Math.max(0, Math.round((sums.get(r.label) || 0) * 100) / 100) }));
  } catch {
    return [];
  }
}
...
export async function getTotals(userId: string): Promise<{
  ...
  try {
    const [profile, inProgress, completed, potentials, seriesWeek, seriesFort, seriesMonth] = await Promise.all([
    ...
    const sumLast = (s: EarningsPoint[]) => (s.length ? s[s.length - 1].amount : 0);
    return {
      ...
      avg_rating: typeof profile.avg_rating === "number" ? profile.avg_rating : 0,
    };
  } catch {
    return {
      completed_count: 0,
