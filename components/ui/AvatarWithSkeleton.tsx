...
export default function AvatarWithSkeleton({
  ...
  const showImg = Boolean(resolvedSrc && !error);
  const common =
    `${sizeClass} rounded-full object-cover ${border ? "border" : ""}`.trim();
  ...
  return (
    <div
      ...
      aria-busy={!loaded && showImg}
    >
      {!loaded && (
        <div
          className={`pointer-events-none absolute inset-0 rounded-full bg-neutral-200 animate-pulse`}
          aria-hidden
        />
      )}
      {showImg ? (
        ...
        <div
          className={`${common} flex items-center justify-center bg-neutral-200 text-neutral-600 ${className}`}
        >
          {getInitials(alt)}
        </div>
      )}
