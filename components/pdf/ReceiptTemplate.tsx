...
const styles = StyleSheet.create({
  ...
  footerTitle: { fontSize: 9, marginBottom: 4 },
  footerDisclaimer: { fontSize: 8, lineHeight: 1.3 },
});
...
function computeTotal(sr: ServerReceipt): number {
  if (typeof sr.payment.amountMXN === "number")
    return sr.payment.amountIsCents
      ? sr.payment.amountMXN / 100
      : sr.payment.amountMXN;
  return sr.payment.total || 0;
}

export function toPdfReceiptData(sr: ServerReceipt): PdfReceiptData {
  const ext = sr as ServerReceipt & {
    meta?: { service_description?: string | null };
  };
  const total0 = computeTotal(sr);
  let servicio =
    typeof sr.payment.subtotal === "number" ? sr.payment.subtotal : 0;
  let comision = 0;
  ...
  if (!servicio && (comision || iva))
    servicio = Math.max(0, total0 - comision - iva);
  ...
  if (total !== round2(servicio + comision + iva))
    total = round2(servicio + comision + iva);
  ...
  return {
    ...
    client: {
      ...
      email: sr.customer?.email ?? null,
    },
    professional: { name: sr.service?.professionalName || "" },
    service: {
      title: sr.service?.title || "Servicio",
      description: ext.meta?.service_description ?? null,
    },
...
export function ReceiptTemplate({
  ...
  const candidate =
    (data as ServerReceipt)?.business?.logoUrl ||
    (baseUrl ? `${baseUrl}/brand/handi-logo.png` : "/brand/handi-logo.png");
  const useImage = candidate && /\.(png|jpg|jpeg|gif)$/i.test(candidate);
  const fallbackLogo = baseUrl
    ? `${baseUrl}/images/LOGO_HANDI_DB.png`
    : "/images/LOGO_HANDI_DB.png";
  const logo = useImage ? candidate : fallbackLogo;
  const disclaimer =
    "Este pago está retenido por Handi hasta que el cliente corrobore que los servicios del profesional se han llevado a cabo con éxito, para cualquier duda o aclaración favor de contactarse al +52 81 3087 8691 o a soporte@handi.mx y con gusto lo atenderemos.";
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.serviceCard} wrap={false}>
          ...
          {vm.service.description ? (
            <Text style={styles.serviceDesc} maxLines={12}>
              {vm.service.description}
            </Text>
          ) : null}
        </View>
        ...
        <View style={styles.breakdown} wrap={false}>
          <View style={styles.breakdownRow}>
            ...
            <Text style={styles.breakdownAmount}>
              {formatCurrencyMXN(vm.amounts.servicio)}
            </Text>
          </View>
          <View style={styles.breakdownRow}>
            <Text style={styles.breakdownLabel}>Comisión</Text>
            <Text style={styles.breakdownAmount}>
              {formatCurrencyMXN(vm.amounts.comision)}
            </Text>
          </View>
          <View style={styles.breakdownRow}>
            <Text style={styles.breakdownLabel}>IVA</Text>
            <Text style={styles.breakdownAmount}>
              {formatCurrencyMXN(vm.amounts.iva)}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.totalBar} wrap={false}>
          ...
          <Text style={styles.totalValue}>
            {formatCurrencyMXN(vm.amounts.total)}
          </Text>
        </View>

        {/* Footer */}
        <View style={styles.footer} fixed>
          <Text style={styles.footerTitle}>Handi � Recibo de pago</Text>
