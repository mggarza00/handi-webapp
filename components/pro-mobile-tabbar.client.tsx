...
import { shouldShowProMobileTabbar } from "@/lib/routing/mobile-tabbar-visibility";

type ProMobileTabbarProps = {
  isClientPro?: boolean;
};

export default function ProMobileTabbar(props: ProMobileTabbarProps) {
  const pathname = usePathname() || "";
  const shouldShow = shouldShowProMobileTabbar({
    effectiveRole: "pro",
    isClientPro: props.isClientPro === true,
    pathname,
  });
  ...
  return (
    <nav
      id="pro-mobile-tabbar"
      className="fixed bottom-0 left-0 right-0 z-40 block md:hidden border-t border-border bg-neutral-50/80 dark:bg-neutral-900/40 backdrop-blur-md"
    >
      <div className="mx-auto flex max-w-5xl items-center justify-around px-4 py-2">
        <Link
          href="/requests/explore"
          className={`${base} ${isExplore ? active : ""}`.trim()}
        >
          <Image
        ...
        <Link
          href="/pro/calendar"
          className={`${base} ${isCalendar ? active : ""}`.trim()}
        >
          <Image
        ...
        <Link
          href="/applied"
          className={`${base} ${isApplied ? active : ""}`.trim()}
        >
          <Image
