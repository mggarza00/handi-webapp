...
const TOTAL_STEPS = 3;
const STEP_ERROR_GROUPS: Array<{ step: number; fields: Array<keyof CreateRequestFormState["errors"] | string> }> = [
...
export default function NewRequestStepper({ className, onSuccess, showStatus = true }: NewRequestStepperProps) {
  ...
  useEffect(() => {
    ...
    for (const group of STEP_ERROR_GROUPS) {
      if (group.fields.some((field) => Boolean((errors as Record<string, string | undefined>)[field]))) {
    ...
    <div className={cn("w-full", className)}>
      <Stepper
        ...
        onFinalStepCompleted={async () => {
          ...
          if (!ok) {
            setStatus("No se pudo enviar la solicitud. Revisa los campos e intenta de nuevo.");
      ...
      >
        <Step label="Describe lo que necesitas">
          <div className="space-y-4">
            <div>
              <h2 className="text-lg font-semibold text-slate-900">Paso 1 - Describe lo que necesitas</h2>
              <p className="text-sm text-slate-600">Completa título, descripción y categoría para clasificar tu solicitud.</p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                {errors?.subcategory ? <p className="text-xs text-red-600">{errors.subcategory}</p> : null}
              </div>
            </div>

            {isAutoCategorizing ? (
              <Badge variant="secondary" className="mt-2 inline-flex items-center gap-2 bg-slate-100 text-xs text-slate-600">
            ...
            {showCategoriesWarning ? (
              <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                No hay categorías disponibles. Revisa la tabla <code>categories_subcategories</code> en Supabase.
              </div>
            ) : null}
          </div>
        </Step>
        ...
        <Step label="Ubicación y contacto">
          <div className="space-y-4">
            <div>
              <h2 className="text-lg font-semibold text-slate-900">Paso 2 - Ubicación y contacto</h2>
              <p className="text-sm text-slate-600">Incluye la ciudad y la dirección usando la búsqueda o el mapa.</p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              >
                <SelectTrigger className={cn(errors?.city && errorFieldClass)} aria-invalid={errors?.city ? true : undefined}>
            ...
            <div className="space-y-1.5">
              ...
              ((address.trim().length >= 3 && addrSuggestions.length > 0) ||
                ...
                <div className="mt-2 max-h-60 w-full overflow-auto rounded-md border bg-white shadow-sm">
                  {(address.trim().length >= 3 ? addrSuggestions : recentAddrs).map((item: AddressSuggestion, idx) => {
                    ...
                    return (
                      <div key={(item.id || item.address) + String(idx)} className="flex items-center gap-2 px-2 py-2 hover:bg-slate-50">
                        <button
                          type="button"
                          onMouseDown={(event) => {
                        ...
                        {saved ? (
                          <button
                            ...
                            onMouseDown={async (event) => {
                              ...
                              try {
                                if (!item.id) return;
                                await fetch(`/api/addresses/book/${encodeURIComponent(String(item.id))}`, {
                                  method: "DELETE",
                                  credentials: "include",
                                });
                              } catch {
              ...
              <p className="text-xs text-slate-500">Puedes escribir la dirección, usar la búsqueda o elegirla en el mapa.</p>
              {!address && coords ? (
        ...
        <Step label="Datos adicionales">
          <div className="space-y-4">
            <div>
              <h2 className="text-lg font-semibold text-slate-900">Paso 3 - Datos adicionales</h2>
              <p className="text-sm text-slate-600">Completa presupuesto, fecha, condiciones y adjunta imágenes del sitio.</p>
            </div>
            ...
            <div className="flex flex-wrap gap-4 md:gap-6 justify-between">
              <div className="space-y-1.5 flex-1 min-w-[150px] max-w-[150px]">
                ...
                <Input
                  ...
                  className={cn("w-full text-center", errors?.required_at && errorFieldClass)}
              ...
              <div className="space-y-1.5 flex-1 min-w-[150px] max-w-[150px]">
                ...
                <div className="relative w-full">
                  ...
                  <Input
                    ...
                    step={100}
                    placeholder="800"
                    className={cn("pl-6 pr-14", errors?.budget && errorFieldClass)}
              ...
              <div className="space-y-1.5 flex-1 min-w-[150px] max-w-[150px]">
                ...
                <div
                  className={cn("rounded-md border border-transparent", errors?.conditions && "border-red-600")}
      ...
      {showStatus ? (
        <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
          <span className="font-medium text-slate-900">Actividad:</span> {status}
        </div>
      ) : null}
