...
export default function MessageList({
  ...
  const [feeCtx, setFeeCtx] = React.useState<{
    offerId: string;
    amount: number | null;
    currency: string;
    title?: string | null;
  ...
  const offerStates = React.useMemo(() => {
    ...
    for (const message of items) {
      ...
      if (typeof offerIdRaw !== "string" || offerIdRaw.trim().length === 0)
        continue;
      ...
      if (message.messageType === "offer") {
        ...
        if (typeof rawDescription === "string" && rawDescription.length)
        ...
        if (
          typeof serviceDateRaw === "string" &&
          serviceDateRaw.trim().length
        ) {
  ...
  function renderOffer(message: Item) {
    ...
    const state = offerStates.get(baseOfferId) ?? {
      ...
      amount:
        typeof payload.amount === "number"
          ? payload.amount
          : Number(payload.amount ?? NaN),
      currency: typeof payload.currency === "string" ? payload.currency : "MXN",
      serviceDate:
        typeof payload.service_date === "string" ? payload.service_date : null,
    ...
    const resolvedOfferId =
      extractOfferId(offerObj) ??
      (typeof offerObjRecord.id === "string"
        ? (offerObjRecord.id as string)
        : state.offerId);
    ...
    const formattedAmount =
      formatCurrency(state.amount, state.currency || "MXN") ?? message.body;
  ...
  function renderBody(message: Item) {
          ...
          {!hasAttachment && canRenderRemoteImage ? (
            <div className="pt-1">
              <button
                ...
                onClick={() =>
                  setQuoteLightbox(
                    `/api/quotes/${encodeURIComponent(qid)}/image`,
                  )
                }
    ...
    if (
      ...
      const nested =
        payloadRecord.location && typeof payloadRecord.location === "object"
