...
const BASE_URL = process.env.PREVIEW_URL || process.env.BASE_URL || `http://localhost:${PORT}`;
...
async function main() {
  ...
  if (hasExternal) {
    await run('node', ['scripts/snapshots.js'], { env: { ...process.env, BASE_URL } });
    // Lighthouse pass (non-blocking)
    try {
      await run('npx', ['lhci', 'autorun', '--config=lighthouserc.cjs', '--collect.numberOfRuns', '1'], { env: { ...process.env, BASE_URL } });
    } catch (err) {
