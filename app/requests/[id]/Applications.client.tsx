...
export default function ApplicationsClient({ requestId, createdBy }: Props) {
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok)
          throw new Error(
            json?.error || "No se pudieron cargar las postulaciones",
          );
        ...
        setAmountEdits((prev) => {
          const next = { ...prev };
          for (const a of list) {
            if (next[a.id] === undefined && requestBudget != null)
              next[a.id] = String(requestBudget);
          }
          return next;
        });
      } catch (e) {
  ...
  if (!items?.length)
    return <p className="text-sm text-gray-600">Aún no hay postulaciones.</p>;
          ...
          <li key={a.id} className="p-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                {a.note && (
                  <p className="text-sm mt-1 whitespace-pre-line">{a.note}</p>
                )}
              </div>
              {createdBy &&
                me &&
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        if (!res.ok)
                          return toast.error(
                            agr?.error || "No se pudo crear el acuerdo",
                          );
                        // Marcar acuerdo como aceptado
                        const res2 = await fetch(
                          `/api/agreements/${agr?.data?.id}`,
                          {
                            method: "PATCH",
                            headers: {
                              "Content-Type": "application/json; charset=utf-8",
                      ...
                      onClick={async () => {
                        ...
                        const res2 = await fetch(`/api/stripe/checkout`, {
                          ...
                          body: JSON.stringify({
                            request_id: requestId,
                            amount_mxn: amount,
                            agreement_id: agr?.data?.id,
                          }),
                        });
                        const chk = await res2.json();
                        if (!res2.ok || !chk?.url)
                          return toast.error(
                            chk?.error || "No se pudo iniciar el checkout",
                          );
            ...
            <button
              ...
              onClick={async () => {
                ...
                const res = await fetch(
                  `/api/applications/${confirmTarget.id}`,
                  {
                    method: "PATCH",
                    headers: {
                      "Content-Type": "application/json; charset=utf-8",
                ...
                const j = await res.json();
                if (!res.ok)
                  return toast.error(j?.error || "Operación fallida");
                setItems(
                  (prev) =>
                    prev?.map((it) =>
