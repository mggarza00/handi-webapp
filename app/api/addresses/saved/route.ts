...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!hasEnv) {
      ...
      return NextResponse.json({ ok: false, error: "supabase_not_configured" }, { status: 500, headers: JSONH });
    }
    ...
    if (!ctype.toLowerCase().includes("application/json")) {
      ...
      return NextResponse.json({ ok: false, error: "invalid_content_type" }, { status: 400, headers: JSONH });
    }
    ...
    const body = await req.json().catch(() => ({}));
    const address_line = typeof body?.address_line === "string" ? body.address_line.trim() : "";
    const address_place_id =
      typeof body?.address_place_id === "string" ? body.address_place_id.trim() : null;
    const lat =
    ...
    if (!address_line) {
      ...
      return NextResponse.json({ ok: false, error: "address_line_required" }, { status: 422, headers: JSONH });
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ ok: false, error: "rpc_failed" }, { status: 400, headers: JSONH });
    }
    ...
    return NextResponse.json({ ok: true, id: data ?? null }, { status: 200, headers: JSONH });
  } catch (err) {
    if (process.env.NODE_ENV !== "production") {
      // eslint-disable-next-line no-console
      console.error("[addresses/saved] unknown", err);
    }
    return NextResponse.json({ ok: false, error: "unknown" }, { status: 500, headers: JSONH });
  }
}
