...
import { editCampaignPlacementCopy } from "@/lib/campaigns/repository";
import {
...
export async function POST(
  ...
  try {
    ...
    const result = await editCampaignPlacementCopy({
      admin,
      campaignMessageId,
    ...
    await logAudit({
      actorId: gate.userId,
      action: "PLACEMENT_COPY_MANUAL_OVERRIDE",
      entity: "campaign_drafts",
      entityId: result.draftId,
      meta: {
        note: `Placement copy manually overridden for ${placementId}.`,
    ...
    const message =
      error instanceof Error ? error.message : "failed to edit placement copy";
    return NextResponse.json(
