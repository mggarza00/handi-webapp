...
export async function POST(req: Request) {
  try {
    ...
    const record = {
      ...
      company_doc_incorporation_url:
        (p.uploads as Record<string, unknown>).company_doc_incorporation_url ??
        null,
      company_csf_url:
        (p.uploads as Record<string, unknown>).company_csf_url ?? null,
      company_rep_id_front_url:
        (p.uploads as Record<string, unknown>).company_rep_id_front_url ?? null,
      company_rep_id_back_url:
        (p.uploads as Record<string, unknown>).company_rep_id_back_url ?? null,
    ...
    if (!inserted) {
      const { error } = await (supabase as any)
        ...
        .insert([record]);
      if (!error) inserted = true;
      else insertErr = insertErr || error;
    }
    ...
    if (!inserted) {
      const detail =
        (insertErr as { message?: string } | null)?.message || "insert_failed";
      try {
        console.error("pro_applications insert failed:", insertErr);
      } catch {}
      return NextResponse.json(
  ...
  ${p.subcategories && p.subcategories.length ? `<p><b>Subcategorías:</b> ${(p.subcategories as string[]).map(escapeHtml).join(", ")}</p>` : ""}
    ...
    const mailRes = await sendEmail({ to: adminRecipients, subject, html });
    if (!mailRes.ok) {
      // Log para diagnóstico en Vercel sin interrumpir el flujo
      console.warn("[email] pro-application notify failed", {
        error: mailRes.error,
        hint: mailRes.hint,
      });
    }
    return NextResponse.json(
      { ok: true, emailOk: mailRes.ok },
