...
export async function POST(req: Request) {
  try {
    ...
    if (String(convLite.pro_id) !== user.id)
      return NextResponse.json(
    ...
    const [proProfile, clientProfile] = await Promise.all([
      ...
      admin
        .from("profiles")
        .select("full_name, email")
        .eq("id", convLite.customer_id)
        .maybeSingle(),
    ]);

    // Fetch request meta for service title
    ...
    try {
      ...
      if (reqId) {
        ...
        if (req) {
          const r: any = req;
          requestTitle =
            typeof r.title === "string" && r.title.trim().length
              ? r.title
              : null;
        }
      }
    ...
    try {
      png = await renderQuotePNG({
        ...
        professional: {
          name: (proProfile.data as any)?.full_name ?? null,
          email: (proProfile.data as any)?.email ?? null,
        },
        client: {
          name: (clientProfile.data as any)?.full_name ?? null,
          email: (clientProfile.data as any)?.email ?? null,
        },
        currency: body.currency || "MXN",
    ...
    async function mergeMessagePayload(
      mid: string,
      ...
      const { data: row } = await admin
        ...
        .maybeSingle();
      const current =
        row && typeof (row as any).payload === "object" && (row as any).payload
          ? ((row as any).payload as Record<string, unknown>)
          : {};
    ...
    const folioForFile = (
      folio && folio.trim().length ? folio : shortId
    ).replace(/[^A-Z0-9_-]+/gi, "-");
    ...
    if (!uploadedKey) {
      ...
      try {
        ...
        const key = messageId
          ...
          : buildStorageKey(
              ["conversation", conversationId, quoteId],
              fileName,
            );
    ...
    if (uploadedKey) {
      await (admin.from("quotes") as any)
        .update({ image_path: uploadedKey })
        .eq("id", quoteId);
      let mid = messageId;
      if (!mid) {
      ...
      if (mid && messageId) {
        await mergeMessagePayload(mid, { image_path: uploadedKey });
      }
      if (mid) {
