    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (url && serviceRole) {
          ...
          const onsiteIdMeta = (
            (session.metadata as any)?.onsite_quote_request_id ||
            (session.metadata as any)?.onsite_request_id ||
            ""
          ).trim();
          const requestIdFromMeta = (
            session.metadata?.request_id ||
            (session.metadata as any)?.requestId ||
            ""
          ).trim();
          const proIdFromMeta = ((session.metadata as any)?.proId || "").trim();
          const scheduledDateMeta = (
            (session.metadata as any)?.scheduled_date || ""
          ).trim();
          ...
          try {
            if (payment_intent_id) {
              ...
              const invId: string | undefined =
                latestCharge?.invoice || undefined;
              if (invId) {
                try {
                  const inv = await stripe.invoices.retrieve(invId);
                  const taxFromArray = Array.isArray(
                    (inv as any).total_tax_amounts,
                  )
                    ? ((inv as any).total_tax_amounts as any[]).reduce(
                        (acc, ta) => acc + Number(ta.amount || 0),
                        0,
                      )
                    : 0;
                  invoiceTax = Number.isFinite(taxFromArray)
                    ? taxFromArray
                    : Number((inv as any).tax || 0);
                } catch {}
              }
              const sessTax = Number(
                (session as any).total_details?.amount_tax || 0,
              );
              iva_cents =
                (invoiceTax || 0) > 0
                  ? invoiceTax
                  : Number.isFinite(sessTax)
                    ? sessTax
                    : 0;
              const received = Number(
                (pi as any).amount_received || (pi as any).amount || 0,
              );
              total_cents = Number.isFinite(received)
                ? received
                : Number(session.amount_total || 0);
              if (!Number.isFinite(total_cents)) total_cents = 0;
              service_cents = Math.max(
                0,
                total_cents - commission_cents - iva_cents,
              );
              const chargeReceipt =
          ...
          try {
            const mc = Number(
              (session.metadata as any)?.commission_cents ?? NaN,
            );
            ...
            if (Number.isFinite(mc) && commission_cents === 0)
              commission_cents = mc;
          ...
          if (offerId && (commission_cents === 0 || iva_cents === 0)) {
            try {
              const { data: offRow2 } = await createClient<Database>(
                url!,
                serviceRole!,
              )
                .from("offers")
              ...
              if (baseCents > 0) {
                const feeC = Math.min(
                  150000,
                  Math.max(5000, Math.round(baseCents * 0.05)),
                );
          ...
          if (total_cents === 0)
            total_cents = Number(session.amount_total || 0) || 0;
          if (iva_cents === 0) {
            const sessTax2 = Number(
              (session as any).total_details?.amount_tax || 0,
            );
          ...
          try {
            ...
            if (offerId) {
              const { data: offRow } = await admin
                .from("offers")
                .select("conversation_id, client_id, professional_id")
                .eq("id", offerId)
                .maybeSingle();
              const convId2 = (offRow as any)?.conversation_id as
                | string
                | undefined;
              ...
              if (!requestId && convId2) {
                const { data: conv2 } = await admin
                  ...
                  .eq("id", convId2)
                  .maybeSingle();
                requestId = ((conv2 as any)?.request_id || null) as
                  | string
                  | null;
              }
            }
          ...
          try {
            if (offerId) {
              const { data: off2 } = await admin
                .from("offers")
                .select("conversation_id, client_id")
                .eq("id", offerId)
                .maybeSingle();
              const convIdAttach = (off2 as any)?.conversation_id as
                | string
                | undefined;
              const senderIdAttach = (off2 as any)?.client_id as
                | string
                | undefined;
              if (receiptIdPersist && convIdAttach && senderIdAttach) {
                ...
                let messageId: string | undefined =
                  Array.isArray(haveMsg) && haveMsg.length
                    ? ((haveMsg[0] as any).id as string)
                    : undefined;
                if (!messageId) {
                ...
                try {
                  const row = await getReceiptForPdf(
                    admin as any,
                    receiptIdPersist,
                  );
                  if (row) {
                    ...
                    const sum =
                      Math.round(
                        (servicio + comision + iva + Number.EPSILON) * 100,
                      ) / 100;
          ...
          if (offerId) {
            ...
            try {
              const { data: off } = await admin
                ...
                .eq("id", offerId)
                .single();
              const convId = (off as any)?.conversation_id as
                | string
                | undefined;
              ...
              if (convId && clientId) {
                ...
                if (!hasPaid) {
                  ...
                  if (scheduledDateMeta)
                    whenStr = `${scheduledDateMeta}${scheduledTimeMeta ? ` ${scheduledTimeMeta}` : ""}`;
                ...
                try {
                  ...
                  const { data: conv } = await admin
                    ...
                    .eq("id", convId)
                    .single();
                  const requestId = (conv as any)?.request_id as
                    | string
                    | undefined;
                  if (requestId) {
                    ...
                    if (serviceDateIso) {
                      try {
                        (patch as any).required_at = serviceDateIso;
                      } catch {}
                    }
                    if (scheduledDateMeta) {
                      (patch as any).scheduled_date = scheduledDateMeta;
                      if (scheduledTimeMeta)
                        (patch as any).scheduled_time =
                    ...
                    const { data: reqRow } = await admin
                      .from("requests")
                      .select(
                        "title,address_line,address_place_id,address_lat,address_lng,scheduled_date,scheduled_time",
                      )
                      .eq("id", requestId)
                      .single();
                    const address_line = (reqRow as any)?.address_line as
                      | string
                      | null;
                    const address_lat = (reqRow as any)?.address_lat as
                      | number
                      | null;
                        ...
                        await (admin as any).from("pro_calendar_events").upsert(
                          {
                            ...
                            title: reqTitle,
                            scheduled_date:
                              scheduled_date || scheduledDateMeta || null,
                            scheduled_time:
                              scheduled_time ||
                              (scheduledTimeMeta as any) ||
                              null,
          ...
          if (!offerId && requestIdFromMeta) {
            try {
              ...
              const { data: convs } = await admin
                ...
                .limit(1);
              const convId =
                Array.isArray(convs) && convs.length
                  ? ((convs[0] as any).id as string)
                  : null;
              const { data: reqRow } = await admin
                .from("requests")
                .select(
              ...
              if (convId && clientId) {
                ...
                const scheduled_time = (reqRow as any)?.scheduled_time as
                  ...
                  | null;
                const reqTitle =
                  ((reqRow as any)?.title as string | undefined) || "Servicio";
                // Upsert calendar event
                try {
                  const proId =
                    proIdFromMeta ||
                    (Array.isArray(convs) && convs.length
                      ? ((convs[0] as any).pro_id as string | undefined)
                      : undefined);
                  if (proId) {
                    await (admin as any).from("pro_calendar_events").upsert(
                      {
      ...
      case "payment_intent.succeeded": {
        ...
        if (url && serviceRole) {
          const admin = createClient<Database>(url, serviceRole);
          const meta = (intent.metadata || {}) as Record<
            string,
            string | undefined
          >;
          const offerId = (
          ...
          let conversationId: string | null = convMeta || null;
          let requestId: string | null =
            ((meta["request_id"] as string | undefined) || "").trim() || null;
          const proIdMeta =
            ((meta["proId"] as string | undefined) || "").trim() || null;
          const scheduledDateMeta = (
            (meta["scheduled_date"] as string | undefined) || ""
          ).trim();
          const scheduledTimeMeta = (
          ...
          let receipt_url: string | null = null;
          try {
            const latestCharge: any = (intent as any).latest_charge || null;
            if (latestCharge && typeof latestCharge === "string") {
              const charge = await stripe.charges.retrieve(latestCharge);
              receipt_url =
                typeof (charge as any)?.receipt_url === "string"
                  ? ((charge as any).receipt_url as string)
                  : null;
            } else if (latestCharge && typeof latestCharge === "object") {
              const r = (latestCharge as any)?.receipt_url;
              if (typeof r === "string") receipt_url = r;
            }
          } catch {
            /* ignore */
          }
          let total_cents = Number(
            (intent.amount_received as number | undefined) ??
              intent.amount ??
              0,
          );
          if (!Number.isFinite(total_cents)) total_cents = 0;
          ...
          let service_cents = 0;
          try {
            const feeMeta = Number((meta as any)?.commission_cents ?? NaN);
            const ivaMeta = Number((meta as any)?.iva_cents ?? NaN);
            const baseMeta = Number((meta as any)?.base_cents ?? NaN);
            const totalMeta = Number((meta as any)?.total_cents ?? NaN);
            if (Number.isFinite(totalMeta) && totalMeta > 0)
              total_cents = totalMeta;
            if (Number.isFinite(feeMeta)) commission_cents = feeMeta;
            if (Number.isFinite(ivaMeta)) iva_cents = ivaMeta;
            if (Number.isFinite(baseMeta)) service_cents = baseMeta;
          } catch {
            /* ignore */
          }
          if (service_cents > 0) {
            const calc = computeClientTotalsCents(service_cents / 100);
            if (commission_cents === 0) commission_cents = calc.feeCents;
            if (iva_cents === 0) iva_cents = calc.ivaCents;
            if (total_cents === 0) total_cents = calc.totalCents;
          }
          if (service_cents === 0 && total_cents > 0) {
            service_cents = Math.max(
              0,
          ...
          let clientId: string | null = null;
          let professionalId: string | null = proIdMeta || null;
          ...
          if (offerId) {
            try {
              ...
              if (offRow) {
                if (!conversationId)
                  conversationId = (offRow as any)?.conversation_id || null;
                clientId = (offRow as any)?.client_id || null;
                if (!professionalId)
                  professionalId = (offRow as any)?.professional_id || null;
                serviceDateIso = (offRow as any)?.service_date || null;
              }
            } catch {
              /* ignore */
            }
          ...
          if (!requestId && conversationId) {
            try {
              const { data: conv } = await admin
                ...
                .maybeSingle();
              requestId = (conv as any)?.request_id || requestId || null;
              if (!clientId) clientId = (conv as any)?.customer_id || null;
              if (!professionalId)
                professionalId = (conv as any)?.pro_id || null;
            } catch {
              /* ignore */
            }
          }
          ...
          if (requestId) {
            ...
            if (professionalId) {
              (patchReq as any).professional_id = professionalId;
              (patchReq as any).accepted_professional_id = professionalId;
            } else if (proIdMeta) {
          ...
          try {
            const { data: existing } = await admin
              ...
              .limit(1);
            let receiptIdPersist: string | null =
              Array.isArray(existing) && existing.length
                ? (existing[0] as any).id
                : null;
            const record: any = {
          ...
          if (conversationId && clientId && offerId) {
            try {
              const { data: existingMsg } = await admin
                ...
                .limit(1);
              const hasPaid =
                Array.isArray(existingMsg) && existingMsg.length > 0;
              if (!hasPaid) {
                let whenStr: string | null = null;
                if (scheduledDateMeta)
                  whenStr = `${scheduledDateMeta}${scheduledTimeMeta ? ` ${scheduledTimeMeta}` : ""}`;
