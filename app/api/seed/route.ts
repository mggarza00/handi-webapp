...
async function createUser(baseUrl: string, serviceRole: string, email: string): Promise<string> {
  ...
  if (!parsed.success) throw new Error("auth_admin_create_user_invalid_response");
  return parsed.data.id;
}

async function insertOne(baseUrl: string, serviceRole: string, table: string, payload: unknown): Promise<string> {
  ...
  if (!parsed.success || parsed.data.length === 0) throw new Error("postgrest_insert_invalid_response");
