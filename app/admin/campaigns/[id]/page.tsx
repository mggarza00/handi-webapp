...
import CampaignDetailTabNav from "@/components/admin/campaigns/CampaignDetailTabNav";
import CampaignExecutiveOverview from "@/components/admin/campaigns/CampaignExecutiveOverview";
...
import { resolvePlacementCopy } from "@/lib/campaigns/placement-copy";
...
import {
  isCampaignDetailTabAvailable,
  resolveCampaignAdminMode,
...
export default async function AdminCampaignDetailPage({
  ...
  const activeTab = coerceCampaignDetailTab(searchParams.tab, adminMode);
  ...
  const criticalWarnings = buildCampaignCriticalWarnings({
    detail,
    visualReadinessSummary,
    queuedJobs,
  });
  const nextBestAction = resolveCampaignNextBestAction({
    detail,
    visualReadinessSummary,
    queuedJobs,
    criticalWarnings,
  });
  const visualReadinessTone = resolveVisualReadinessTone(
    visualReadinessSummary.overallState,
  );
    ...
    <main className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {adminMode === "advanced" ? (
            <>
              <Button asChild variant="outline">
                <Link
                  href={`/admin/creative-assets?campaignId=${detail.draft.id}`}
                >
                  Creative assets
                </Link>
              </Button>
          ...
          <Button asChild variant="outline">
            <Link
              href={`/admin/campaigns/new?from=${detail.draft.id}${adminMode === "advanced" ? "&mode=advanced" : ""}`}
            >
              Duplicate campaign
            </Link>
          </Button>
      ...
      <CampaignExecutiveOverview
        editorialStatus={detail.draft.status}
        publishStatus={detail.draft.publish_status}
        visualReadinessLabel={labelVisualReadinessState(
          visualReadinessSummary.overallState,
        )}
        readinessTone={visualReadinessTone}
        ...
        actionSlot={
          <>
            <Button asChild variant="outline" size="sm">
              <Link
                href={`/admin/campaigns/${detail.draft.id}?tab=copy&mode=${adminMode}${from ? `&from=${encodeURIComponent(from)}` : ""}${to ? `&to=${encodeURIComponent(to)}` : ""}`}
              >
                Review copy
              </Link>
            </Button>
            <Button asChild variant="outline" size="sm">
              <Link
                href={`/admin/campaigns/${detail.draft.id}?tab=handoff&mode=${adminMode}${from ? `&from=${encodeURIComponent(from)}` : ""}${to ? `&to=${encodeURIComponent(to)}` : ""}`}
              >
                Review handoff
              </Link>
            </Button>
                    ...
                    <div className="mt-4 space-y-2">
                      <div className="flex flex-wrap gap-2">
                        <ProviderMetaBadges
                          providerName={detail.draft.generation_provider}
                          generationMode={
                            detail.draft.provider_metadata.generationMode
                          }
                          model={detail.draft.provider_metadata.model}
                        />
                      </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Automatic QA helps prioritize review. It does not replace
                  human approval.
                </p>
              </div>
              <form
            ...
            <CardContent className="space-y-4">
              ...
              {adminMode === "advanced" ? (
                ...
                <details className="rounded-lg border p-4 text-sm">
                  <summary className="cursor-pointer font-medium">
                    QA details ({detail.draft.qa_report.warnings.length}{" "}
                    warnings, {detail.draft.qa_report.suggestions.length}{" "}
                    suggestions)
                  </summary>
                    ...
                    {detail.draft.channel_plan.length ? (
                      detail.draft.channel_plan.map((step) => (
                        ...
                        >
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">
                            {step.format} | {step.timing} | trigger:{" "}
                            {step.trigger}
                          </p>
                        </div>
                      ))
          ...
          <CardHeader className="flex flex-row flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                ...
                bundle. Exact approved derivatives are preferred, approved
                masters can act as a fallback, and manual overrides stay
                traceable.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <TrackedButtonLink
                ...
                eventParams={campaignBundleTrackingContext}
                analyticsContext={campaignBundleTrackingContext}
              >
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="rounded-lg border p-3 text-sm">
                          ...
                          <p className="mt-2 text-muted-foreground">
                            ...
                            `manifest.json`, `copy.json`, and the selected
                            visual asset for handoff.
                          </p>
                          <div className="mt-3 flex flex-wrap gap-2">
                            ...
                            {channel === "meta" || channel === "google" ? (
                              <>
                                <TrackedButtonLink
                                  href={`/api/admin/campaigns/${detail.draft.id}/paid-draft/${channel}`}
                                  target="_blank"
                                  variant="outline"
                                  eventName="campaign_bundle_viewed"
                                  eventParams={{
                                ...
                                <TrackedButtonLink
                                  href={`/api/admin/campaigns/${detail.draft.id}/paid-draft/${channel}?download=1`}
                                  target="_blank"
                                  variant="outline"
                                  eventName="export_package_downloaded"
                                  eventParams={{
                          ...
                          <div className="rounded-lg border p-3 text-sm">
                            ...
                            {channel === "meta" || channel === "google" ? (
                              <p className="mt-2 text-xs text-muted-foreground">
                                This block is optimized for paid ops handoff. It
                                shows exact vs fallback visual coverage,
                                inherited vs placement-specific copy, and
                                placement-level export/download actions.
                              </p>
                            ) : null}
                            {adminMode === "basic" ? (
                              <p className="mt-2 text-xs text-muted-foreground">
                                Placement-level details stay collapsed in basic
                                mode. Open them only when a placement needs a
                                specific override or warning review.
                              </p>
                            ) : null}
                            <div className="mt-3 space-y-3">
                              {placementCoverage.placements.map((placement) => {
                                const placementDefinition =
                                  getCreativePlacementDefinition(
                                ...
                                const placementCopy = resolvePlacementCopy({
                                  ...
                                  messages: detail.messages,
                                  decisions: detail.variantDecisions,
                                  placementCopies: detail.placementCopies,
                                  ...
                                  >
                                    <div className="flex flex-wrap items-start justify-between gap-3">
                                      <div>
                                        ...
                                        <div className="mt-1 text-xs text-muted-foreground">
                                          {placementDefinition.platformLabel} |{" "}
                                          {placementDefinition.operationalName}
                                        </div>
                                        <div className="mt-1 flex flex-wrap gap-2 text-xs">
                                          <Badge
                                            variant={
                                          ...
                                          <Badge variant="outline">
                                            Copy:{" "}
                                            {placementDefinition.copyStyle}
                                          </Badge>
                                          <Badge variant="outline">
                                            {placementDefinition.placementGroup.replace(
                                              /_/g,
                                              " ",
                                            )}
                                          </Badge>
                                      ...
                                      <div className="text-right text-xs text-muted-foreground">
                                        <div>
                                          {placementDefinition.namingHint}
                                        </div>
                                        <div>
                                    ...
                                    <div className="mt-3 rounded-md border bg-muted/20 p-3 text-xs">
                                      <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        <Badge variant="outline">
                                          {labelPlacementCopySource(
                                            placementCopy.source,
                                          )}
                                        </Badge>
                                        {placementCopy.status === "missing" ? (
                                          <Badge variant="destructive">
                                            Missing
                                          </Badge>
                                        ) : (
                                          <StateBadge
                                            value={placementCopy.status}
                                          />
                                        )}
                                        <Badge variant="outline">
                                          {placementCopy.inheritedFromChannel
                                            ? "Channel fallback"
                                            : "Placement specific"}
                                        </Badge>
                                      </div>
                                      <p className="mt-2 text-muted-foreground">
                                        {placementCopy.summary}
                                      </p>
                                      <p className="mt-2 text-muted-foreground">
                                        {placementDefinition.copyGuidance}
                                      </p>
                                      {placementCopy.content ? (
                                        <div className="mt-2 space-y-1 text-muted-foreground">
                                          <div className="font-medium text-foreground">
                                            {placementCopy.baseVariantName ||
                                              "Channel variant"}
                                          </div>
                                          <div>
                                            {placementCopy.content.headline}
                                          </div>
                                          <div className="line-clamp-3">
                                            {placementCopy.content.body}
                                          </div>
                                          <div className="font-medium text-foreground">
                                            CTA: {placementCopy.content.cta}
                                          </div>
                                          <div>
                                            QA:{" "}
                                            {placementCopy.qaReport
                                              ? `${placementCopy.qaReport.overall_score}/100`
                                              : "Pending"}
                                          </div>
                                        </div>
                                      ) : (
                                        <p className="mt-2 text-muted-foreground">
                                          No placement-level copy is available
                                          yet.
                                        </p>
                                      )}
                                      {placementCopy.warnings.length ? (
                                        <p className="mt-2 text-amber-700">
                                          {placementCopy.warnings.join(" ")}
                                        </p>
                                      ) : null}

                                      {placementCopy.baseMessageId ? (
                                        <div className="mt-3 space-y-3">
                                          <form
                                            action={`/api/admin/campaigns/${detail.draft.id}/placement-copy/generate`}
                                            method="post"
                                            className="space-y-2"
                                          >
                                            <input
                                            ...
                                            <Textarea
                                              name="feedbackNote"
                                              rows={2}
                                              placeholder="Optional guidance for a placement-specific rewrite."
                                            />
                                            <Button
                                              type="submit"
                                              variant="outline"
                                              size="sm"
                                            >
                                              Generate placement copy
                                            </Button>
                                          </form>

                                          <form
                                            action={`/api/admin/campaigns/${detail.draft.id}/placement-copy/edit`}
                                            method="post"
                                          ...
                                          >
                                            ...
                                            <Input
                                              ...
                                              placeholder="Placement headline"
                                            />
                                            <Textarea
                                              name="body"
                                              rows={3}
                                              ...
                                              placeholder="Placement body"
                                            />
                                            <Input
                                              name="cta"
                                              defaultValue={
                                                placementCopy.content?.cta || ""
                                              }
                                              placeholder="Placement CTA"
                                            />
                                            <Textarea
                                              name="rationaleNote"
                                              rows={2}
                                              defaultValue={
                                                placementCopy
                                                  .pendingPlacementCopy
                                                  ?.rationale_parts.note || ""
                                              }
                                              placeholder="Why does this placement need different copy?"
                                            />
                                            <Button
                                              type="submit"
                                              variant="outline"
                                          ...
                                          {placementCopy.pendingPlacementCopy ||
                                          placementCopy.placementMessageId ? (
                                            <div className="flex flex-wrap gap-2">
                                              <form
                                                action={`/api/admin/campaigns/${detail.draft.id}/placement-copy/approve`}
                                                method="post"
                                              >
                                                <input
                                                  type="hidden"
                                              ...
                                              <form
                                                action={`/api/admin/campaigns/${detail.draft.id}/placement-copy/reject`}
                                                method="post"
                                              >
                                                <input
                                                  type="hidden"
                                    ...
                                    {placementDefinition.placementFamily ===
                                    "paid" ? (
                                      <div className="mt-3 rounded-md border border-amber-200 bg-amber-50/60 p-3 text-xs text-amber-900">
                                        ...
                                        <div className="mt-2 space-y-1">
                                          <div>
                                            Asset coverage:{" "}
                                            {placement.isExact
                                              ? "Exact format selected"
                                              : placement.selectedAsset
                                                ? "Fallback asset selected"
                                                : "No asset selected"}
                                          </div>
                                          <div>
                                            Copy source:{" "}
                                    ...
                                    <div className="mt-3 flex flex-wrap gap-2">
                                      ...
                                      "paid" ? (
                                        <>
                                          <TrackedButtonLink
                                            href={`/api/admin/campaigns/${detail.draft.id}/paid-draft/${channel}?placement=${placement.placementId}`}
                                            target="_blank"
                                            size="sm"
                                            variant="outline"
                                            eventName="campaign_bundle_viewed"
                                          ...
                                          <TrackedButtonLink
                                            href={`/api/admin/campaigns/${detail.draft.id}/paid-draft/${channel}?placement=${placement.placementId}&download=1`}
                                            target="_blank"
                                            size="sm"
                                            variant="outline"
                                            eventName="export_package_downloaded"
      ...
      {activeTab === "handoff" && adminMode === "advanced" ? (
        <>
          <div className="grid gap-4 xl:grid-cols-[0.8fr_1.2fr]">
            <Card>
              ...
              <CardContent className="space-y-4 text-sm">
                ...
                <div className="space-y-2 text-muted-foreground">
                  <p>
                    Publishing stays manual and only runs after approval.
                    Supported channels can be sent live, drafted, or exported
                    depending on the connector.
                  </p>
                  <p>
                    Current state:{" "}
                    {labelPublishStatus(detail.draft.publish_status)}.
                  </p>
                </div>
                ...
                <div className="rounded-lg border p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant="outline">
                      {analytics.signal_summary.manual_source_count}{" "}
                      manual/snapshot sources
                    </Badge>
                  </div>
                  <p className="mt-2 text-xs text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="text-sm text-muted-foreground">
                  Email can go live when explicit recipients are provided. Push
                  can go live only with explicit user IDs and configured VAPID
                  keys. WhatsApp, landing, Meta, and Google stay in draft/export
                  mode in this phase. Scheduling never skips approval or publish
                  readiness.
                </div>
                ...
                <div className="rounded-lg border p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <form
                      action="/api/admin/publish-jobs/run-due"
                      method="post"
                    >
                      <input
                        type="hidden"
                    ...
                    <div key={job.id} className="rounded-lg border p-4">
                      ...
                      {jobPerformance ? (
                        <div className="mt-3 flex flex-wrap gap-2 text-xs">
                          <Badge variant="outline">
                            Deliveries{" "}
                            {formatInteger(jobPerformance.deliveries)}
                          </Badge>
                          <Badge variant="outline">
                          ...
                          <Badge variant="outline">
                            Conversions{" "}
                            {formatInteger(jobPerformance.conversions)}
                          </Badge>
      ...
      {activeTab === "handoff" && adminMode === "basic" ? (
        <Card>
          <CardHeader>
            <CardTitle>Advanced publish controls are parked</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 text-sm text-muted-foreground">
            <p>
              Basic mode focuses on selected copy, approved visuals, and export
              handoff. Queue controls, scheduling, and publish job history stay
              in advanced mode so the MVP flow stays lighter.
            </p>
            <Button asChild variant="outline" size="sm">
              <Link
                href={`/admin/campaigns/${detail.draft.id}?tab=handoff&mode=advanced${from ? `&from=${encodeURIComponent(from)}` : ""}${to ? `&to=${encodeURIComponent(to)}` : ""}`}
              >
                Open advanced publish tools
              </Link>
            </Button>
          </CardContent>
        </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3 text-sm">
                  ...
                  <ComparisonLine
                    ...
                    previous={formatRate(
                      analytics.comparison.previous.open_rate,
                    )}
            ...
            <section key={channel} className="space-y-4">
              ...
              <Accordion defaultValue={[]}>
                {messages.map((message) => {
                  ...
                  const messagePerformance = messagePerformanceById.get(
                    message.id,
                  );
                          ...
                          <div className="space-y-4">
                            ...
                            <Card>
                              ...
                              <CardContent className="space-y-3 text-sm">
                                <div className="flex flex-wrap gap-2">
                                  <StateBadge
                                    value={message.qa_report.qa_status}
                                  />
                                  <StateBadge
                                ...
                                <div className="grid gap-3 md:grid-cols-2">
                                  ...
                                  <MetaLine
                                    label="Clarity"
                                    value={String(
                                      message.qa_report.clarity_score,
                                    )}
                                  />
                                  <MetaLine
                                    label="CTA"
                                ...
                                <div>
                                  ...
                                  {message.qa_report.warnings.length ? (
                                    <div className="mt-2 space-y-2">
                                      {message.qa_report.warnings.map(
                                        (warning) => (
                                          <div
                                            key={warning}
                                            className="rounded-lg border p-3"
                                    ...
                                    >
                                      <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        <ProviderMetaBadges
                                          ...
                                          model={
                                            version.provider_metadata.model
                                          }
                                        />
...
function coerceCampaignDetailTab(
  value: string | undefined,
  mode: CampaignAdminMode,
): CampaignDetailTabKey {
  if (isCampaignDetailTabAvailable(value, mode)) return value;
  return "overview";
}

function buildCampaignCriticalWarnings(args: {
  detail: Awaited<ReturnType<typeof getCampaignDetail>>;
  visualReadinessSummary: ReturnType<typeof evaluateCampaignVisualReadiness>;
  queuedJobs: Array<{ queue_status: string; last_error: string | null }>;
}) {
  if (!args.detail) return [];

  const warnings: string[] = [];

  if (args.detail.draft.qa_report.qa_status === "high_risk") {
    warnings.push(
      "QA marked this campaign as high risk. Review copy before any handoff.",
    );
  }

  if (args.visualReadinessSummary.overallState === "blocked") {
    warnings.push(
      "Visual coverage is blocked for at least one active channel or placement.",
    );
  }

  if (args.detail.draft.last_publish_error) {
    warnings.push(args.detail.draft.last_publish_error);
  }

  if (args.queuedJobs.some((job) => job.queue_status === "failed")) {
    warnings.push(
      "At least one queued publish job failed and needs operator review.",
    );
  }

  if (
    args.detail.draft.status === "approved" &&
    args.detail.draft.publish_status !== "published" &&
    args.visualReadinessSummary.blockedCount > 0
  ) {
    warnings.push(
      "Campaign is approved but still lacks publish-safe creative coverage.",
    );
  }
...
function resolveCampaignNextBestAction(args: {
  detail: NonNullable<Awaited<ReturnType<typeof getCampaignDetail>>>;
  visualReadinessSummary: ReturnType<typeof evaluateCampaignVisualReadiness>;
  queuedJobs: Array<{ queue_status: string }>;
  criticalWarnings: string[];
}) {
  if (args.detail.draft.status !== "approved") {
    return "Finish editorial review first: validate checklist, review copy, and approve or request changes.";
  }

  if (args.visualReadinessSummary.overallState === "blocked") {
    return "Resolve missing or blocked visual coverage before relying on handoff or assisted publish.";
  }

  if (args.detail.draft.publish_status !== "ready_to_publish") {
    return "Move to Export / Handoff and mark the campaign ready once copy, visuals, and readiness look correct.";
  }

  if (args.queuedJobs.some((job) => job.queue_status === "failed")) {
    return "Review failed publish jobs, decide whether to retry, and confirm the latest selected copy and creative bundle.";
  }

  if (args.criticalWarnings.length) {
    return "Check the highlighted warnings before continuing with exports or queue actions.";
  }

  return "Campaign looks operationally healthy. Use the domain tabs to inspect copy, placements, and exports as needed.";
}

function resolveVisualReadinessTone(
  state: ReturnType<typeof evaluateCampaignVisualReadiness>["overallState"],
): "good" | "warn" | "bad" {
  if (state === "ready_exact") return "good";
  if (state === "blocked" || state === "missing") return "bad";
  return "warn";
}
