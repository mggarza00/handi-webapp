...
export default async function AdminLayout({
  ...
  const allowDevBypass =
    ...
    ["owner", "admin", "ops", "finance", "support", "reviewer"].includes(
      cookieRole,
    );
  ...
  const allowedByEmail =
    user?.email && seed ? user.email.toLowerCase() === seed : false;
  const allowed =
    allowDevBypass || canAccessAdmin(role, is_admin) || Boolean(allowedByEmail);
  if (!allowed) {
    return (
  ...
  const nav = [
    ...
    },
    {
      ...
      label: "Ofertas",
      icon: CreditCard,
      testId: "admin-nav-offers",
    },
    {
      href: "/admin/requests",
      label: "Solicitudes",
      icon: Boxes,
      testId: "admin-nav-requests",
    },
    {
      href: "/admin/professionals",
      label: "Profesionales",
      icon: ShieldCheck,
      testId: "admin-nav-professionals",
    },
    {
      href: "/admin/pro-applications",
      label: "Postulaciones",
      icon: FileText,
      testId: "admin-nav-pro-applications",
    },
    {
      href: "/admin/pro-changes",
      label: "Cambios de perfil",
      icon: FileText,
      testId: "admin-nav-pro-changes",
    },
    {
      href: "/admin/payments",
      label: "Pagos",
  ...
  return (
    <SidebarProvider>
      ...
      <Sidebar width={300}>
        ...
        <SidebarHeader className="mt-6 mb-2 text-sm uppercase tracking-wider text-muted-foreground">
          Más
        </SidebarHeader>
        <SidebarContent>
          {stubs.map((item) => (
        ...
        <aside className="sticky top-14 hidden h-[calc(100dvh-3.5rem)] shrink-0 border-r md:block bg-background">
          <div className="flex h-full flex-col gap-3 p-3 overflow-y-auto">
            <div className="flex items-center px-2 pt-1 min-h-9">
              <div className="text-sm font-semibold group-data-[collapsed=true]:hidden">
                Administración
              </div>
            ...
            <nav className="flex flex-col gap-1">
              {nav.map((item) => (
                <NavLink
                  ...
                  title={item.label}
                  dataTestId={
                    item.testId ? `${item.testId}-desktop` : undefined
                  }
            ...
            <div className="px-2 pt-3 text-xs uppercase tracking-wider text-muted-foreground group-data-[collapsed=true]:hidden">
              Más
            </div>
